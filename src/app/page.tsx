...
export default function LandingPage() {
  ...
  async function importFromLink() {
    if (!jobUrl.trim()) return alert("Paste a job posting link first.");
    try {
      setImporting(true);
  ...
  async function validateSelectedFile(f: File) {
    ...
    try {
      ...
      if (!data?.chars || data.chars < 20) throw new Error("That file appears to have little or no extractable text.");
  ...
  function onDrop(e: React.DragEvent) {
    ...
    const f = e.dataTransfer.files?.[0]; if (f) { setFile(f); validateSelectedFile(f); }
  }
  ...
  async function handleGenerateAll() {
    ...
    setLoading(true); setStatus("Starting…"); setProgress(10);
    try {
      ...
      const coverPromise = fetch("/api/cover-letter", { method: "POST", body: fdCover })
        .then((r) => r.json()).then((d) => { setProgress(70); setStatus("Drafting cover letter…"); return d; });
      ...
      router.push("/results");
    } catch { alert("Something went wrong generating results."); }
    ...
    <div className="min-h-screen flex flex-col bg-gradient-to-b from-gray-50 to-gray-200 dark:from-gray-950 dark:to-gray-900">
      <main className="flex-1">
        <div className="mx-auto max-w-4xl px-6 pt-10 pb-6 text-center">
          ...
          <p className="mt-2 text-gray-600 dark:text-gray-400">Upload a resume, then add a job description to generate tailored outputs.</p>
        </div>
        ...
        <div className="mx-auto max-w-3xl bg-white dark:bg-gray-950 rounded-3xl shadow-lg p-6 md:p-8 space-y-6 border border-gray-200 dark:border-gray-800">
          <div>
            ...
            >
              ...
              {validationError && <div className="mt-3 text-sm text-red-600 dark:text-red-400">{validationError}</div>}
            ...
            <p className="mt-2 text-xs text-gray-500 dark:text-gray-400 text-center">Max file size 10MB. Supported: PDF, DOCX.</p>
          </div>
            ...
            <>
              <div className="space-y-2">
                ...
                <div className="flex flex-col gap-2 sm:flex-row">
                  <input className={`${inputBase} flex-1`} placeholder="https://careers.company.com/job/12345" value={jobUrl} onChange={(e) => setJobUrl(e.target.value)} type="url" inputMode="url" />
                  <button onClick={importFromLink} disabled={!jobUrl.trim() || importing} className="inline-flex items-center justify-center rounded-xl px-5 py-3 font-medium bg-gray-900 text-white hover:bg-black disabled:opacity-50">
                    {importing ? <Spinner /> : null}
                    {importing ? "Importing…" : "Import from link"}
                  </button>
                </div>
                {importTitle && <p className="text-xs text-gray-600 dark:text-gray-400"><span className="font-semibold">Imported title:</span> {importTitle}</p>}
              </div>
              ...
              <div>
                ...
                <textarea className={`${taBase} h-72`} value={jobDescription} onChange={(e) => setJobDescription(e.target.value)} placeholder="Paste the full job posting here… (or import from a link above)" />
                <div className="text-right text-xs text-gray-500 dark:text-gray-400">{jobDescription.length.toLocaleString()} characters</div>
              </div>

              <div className="space-y-3">
                <button onClick={handleGenerateAll} disabled={!isReadyToGenerate} className={`${btnBase} w-full text-white bg-gradient-to-r from-indigo-600 to-fuchsia-600 hover:from-indigo-700 hover:to-fuchsia-700`}>
                  {loading && <Spinner />}
                ...
                {loading && (
                  <div className="w-full">
                    <div className="h-2 w-full bg-gray-200 dark:bg-gray-800 rounded-full overflow-hidden">
                      <div className="h-2 bg-gradient-to-r from-indigo-600 to-fuchsia-600 transition-all duration-300" style={{ width: `${progress}%` }} />
                    </div>
      ...
      <footer className="bg-gray-100 dark:bg-gray-950 border-t border-gray-200 dark:border-gray-800 py-6 mt-10">
        <div className="mx-auto max-w-4xl px-6 flex flex-col md:flex-row items-center justify-between gap-3 text-sm">
          <div className="text-gray-700 dark:text-gray-300 text-center md:text-left">© {currentYear} Job PowerUp. All rights reserved.</div>
